function[] = Display_Designed_Track3(matrix)
%% the structure of matrix:
% Each row: AP ML DV alpha beta
...
for selected_probe = 1:size(matrix,1)
    ...
    p = [-sind(matrix(selected_probe,5))*cosd(matrix(selected_probe,4)),cosd(matrix(selected_probe,5)),-sind(matrix(selected_probe,5))*sind(matrix(selected_probe,4))];
    ...
    %writetable(borders_table,[save_location '\Probe ' '25' '.csv']);
