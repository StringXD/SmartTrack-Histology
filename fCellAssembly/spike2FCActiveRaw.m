    ...
    for I = 1:114
        ...
        for bin = 1:6
            ...
            if nnz(sel11)>0 
                ...
                currSessConnS1 = [currSessConnS1;fstr{bin}.conn_chain_S1(sel11,:)-lbound,bin*ones(sum(sel11),1),fstr{bin}.pref_chain_S1(sel11,bin),fstr{bin}.pref_chain_S1(sel11,bin+6)];
            end
            if nnz(sel12)>0
                currSessConnS2 = [currSessConnS2;fstr{bin}.conn_chain_S2(sel12,:)-lbound,bin*ones(sum(sel12),1),fstr{bin}.pref_chain_S2(sel12,bin),fstr{bin}.pref_chain_S2(sel12,bin+6)];
            end
        end
        connSum{I,1} = currSessConnS1;
        connSum{I,2} = currSessConnS2;
    end
    save('connSumActive.mat','connSum','-v7.3');
end
%% prepare spike train data
if ~exist('connSum','var')
    load('connSumActive.mat');
end
...
for ssidx = 1:length(connSum)
    ...
    for sample = 1:2
        ...
        for suIdx = 1:length(suList)
            [rows,loc] = find(currData(:,1:2) == suList(suIdx) & currData(:,4:5));
            if isempty(rows)
                continue;
            end
            ...
            for bin = workBins(:)'
                if ~isempty(preCase)
                    ...
                    post = preCase(preCase(:,3) == bin & preCase(:,5),[2,5]);
                else
                    candidate = [suList(suIdx),postCase(1,5)];
                    post = [];
                end
                if ~isempty(postCase)
                    pre = postCase(postCase(:,3) == bin & postCase(:,4),[1,4]);
                else
                    pre = [];
                end
...
save('spike2FCActiveRaw.mat','sumSum','-v7.3');
